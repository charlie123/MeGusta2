...
#pragma unmanaged
extern MyGUI::DirectXPlatform* guiPlatform;
...
{
	guiPlatform->getRenderManagerPtr()->deviceLost();
    HRESULT res = m_pIDirect3DDevice9->Reset(pPresentationParameters);
	guiPlatform->getRenderManagerPtr()->deviceRestore();
...
HRESULT myIDirect3DDevice9::EndScene(void)
{
	...
	MyGUI::DirectXRenderManager* rm = guiPlatform->getRenderManagerPtr();
	rm->drawOneFrame();
