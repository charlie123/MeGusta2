...
typedef HRESULT (WINAPI * DwmEnableComposition_t)(
...
void ResetWindowTitle() {
	HWND hWnd = FindWindowA((char*)0x6F396C, NULL);

	if (hWnd) {
		SetWindowTextA(hWnd, (char*)0x7859C0);
	}
}
...
hostent* WINAPI custom_gethostbyname(const char* name) {
	...
	if (current == log1 || current == match1 || current == blob1) {
		if (!serverName) {
			CIniReader reader(".\\alterIWnet.ini");
			serverName = reader.ReadString("Configuration", "Server", "dlc1-log1.pc.iw4.iwnet.infinityward.com");
		}
...
{
	if (Custom::Hook == nullptr) return;
	Custom::Hook->GUIInited();
}

void ToggleGlobalGUI(bool on)
{
	if (Custom::Hook == nullptr) return;
	Custom::Hook->ToggleGlobalGUI(on);
}
